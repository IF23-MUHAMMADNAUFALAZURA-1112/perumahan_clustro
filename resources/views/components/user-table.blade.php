<table class="min-w-full bg-white border border-gray-200 rounded-lg shadow-md mb-4">
    ...
    <tbody>
        @forelse($users as $user)
            <tr class="border-t border-gray-200">
                ...
                <td class="px-4 py-2 flex gap-2">
                    <form method="POST" action="{{ route('admin.users.toggleAkses', $user->id) }}">
                        @csrf
                    ...
                    <form method="POST" action="{{ route('admin.users.changeRole', $user->id) }}">
                        ...
                        <button class="px-3 py-1 text-sm bg-yellow-500 text-white rounded hover:bg-yellow-600">
                            Jadikan {{ $user->role === 'warga' ? 'Satpam' : 'Warga' }}
                        </button>
                    </form>
                </td>
            </tr>
        @empty
            <tr>
                <td colspan="4" class="px-4 py-2 text-center text-gray-500">Tidak ada pengguna.</td>
            </tr>
        @endforelse
    </tbody>
</table>
